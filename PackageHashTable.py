...
class PackageHashTable:
    ...
    # This initialization method creates our hash table
    ...
    def print_package_hash_table(self, t):
        for i in self.packages_array:
            if t < i.time_loaded:
                ...
                i.print_package("Loaded on truck 2")
            else:
                i.print_package("Delivered")

    # This method imports the package info from the csv file into the hash table
    def import_package_info(self):
        with open('WGUPSPackageFile.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            for row in csv_reader:
                deadline = datetime.combine(date.today(), time(int(row[5]), int(row[6])))
                delay_time = datetime.combine(date.today(), time(int(row[9]), int(row[10])))
                time_placeholder = datetime.combine(date.today(), time(1, 0))
                self.insert_package(Package(int(row[0]), row[1], row[2], row[3], int(row[4]),
                                            deadline, int(row[7]), int(row[8]), delay_time,
                                            row[11], "Undelivered", time_placeholder, time_placeholder))
                if row[11] == "y":
    ...
    # can be shipped out
    def is_appropriate_time_for_special_group(self):
    ...
    def deliver(self):
        ...
        prioritized_list = []
        if self.is_appropriate_time_for_special_group() and not self.package_group_delivered:
            for i in self.package_group:
